...
async def login_for_access_token_controller(
    ...
    login_token = await LoginService.authenticate_user_service(
        request=request, login_form=login_form, db=db
    )
...
async def get_new_token_controller(
    ...
    new_token = await LoginService.refresh_token_service(
        db=db, request=request, refresh_token=payload
    )
...
async def get_captcha_for_login_controller() -> JSONResponse:
    """
    获取登录验证码
    ...
    """
    # 获取验证码
    captcha = await CaptchaService.get_captcha_service()
    log.info("获取验证码成功")
...
async def logout_controller(
    ...
    """
    if await LoginService.logout_service(token=payload):
        log.info("退出成功")
